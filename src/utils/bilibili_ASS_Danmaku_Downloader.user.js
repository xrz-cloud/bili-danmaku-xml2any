...
// 动态导入设置项，并返回ASS弹幕
export var SetConfig = function (xml, FileName, Dconfig) {
  config = Dconfig;
  if (config !== Dconfig) return false;
  const danmaku = parseFile(xml);
  return generateASS(setPosition(danmaku), {
...
var hexAlpha = function (opacity) {
  var alpha = Math.round(0xff * (1 - opacity))
    .toString(16)
    .toUpperCase();
...
var hypot = Math.hypot
  ? Math.hypot.bind(Math)
  : function () {
      return Math.sqrt(
        [0].concat(Array.apply(Array, arguments)).reduce(function (x, y) {
          return x + y * y;
        })
      );
    };
...
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
  ...
  var format = (function () {
    ...
    var r2l = function (line) {
      return (
        "\\move(" +
        [line.poss.x, line.poss.y, line.posd.x, line.posd.y].join(",") +
        ")"
      );
    };
    // 适用于固定位置弹幕
    var fix = function (line) {
      return "\\pos(" + [line.poss.x, line.poss.y].join(",") + ")";
    };
...
var fetchXML = function (cid, callback) {
  GM_xmlhttpRequest({
    ...
    onload: function (resp) {
      var content = resp.responseText.replace(
...
var getCid = function (callback) {
  ...
  try {
    src = document
      .querySelector("#bofqi iframe, #moviebofqi iframe")
      .src.replace(/^.*\?/, "");
  ...
  if (!cid)
    try {
      src = document
        .querySelector("#bofqi embed, #moviebofqi embed")
  ...
  if (!cid)
    try {
      src = document
        .querySelector(
          '#bofqi object param[name="flashvars"], #moviebofqi object param[name="flashvars"]'
        )
        .getAttribute("value");
      cid = Number(src.match(/cid=(\d+)/)[1]);
    } catch (e) {}
...
var showButton = function (count) {
  ...
  var anim = function () {
    ...
    frame++;
    assdown.querySelector("i").style.backgroundPosition =
      "-" + frame * 80 + "px 0";
...
// 由 https://tiansh.github.io/us-danmaku/bilibili/ 导入
